...
export class wangStackLayout extends wangGraphLayout {
  marginTop = 0;
  marginLeft = 0;
  marginRight = 0;
  marginBottom = 0;
    ...
    if (cell != null && parent != null) {
      ...
      for (i = 0; i < childCount; i++) {
        ...
        if (child != cell) {
          ...
          if (bounds != null) {
            let tmp = horizontal ? bounds.x + bounds.width / 2 : bounds.y + bounds.height / 2;
  ...
  getLayoutCells(parent) {
    ...
    if (this.allowGaps) {
      cells.sort((c1, c2) => {
        let geo1 = this.graph.getCellGeometry(c1);
        let geo2 = this.graph.getCellGeometry(c2);
  ...
  execute(parent) {
    if (parent != null) {
      ...
      if (this.graph.isSwimlane(parent)) {
        ...
        let start = wangUtils.getNumber(style, wangConstants.STYLE_STARTSIZE, wangConstants.DEFAULT_STARTSIZE);
        let horz = wangUtils.getValue(style, wangConstants.STYLE_HORIZONTAL, true) == 1;
        ...
        for (let i = 0; i < cells.length; i++) {
          ...
          if (geo != null) {
            ...
            if (!this.borderCollapse) {
              ...
              sw = wangUtils.getNumber(childStyle, wangConstants.STYLE_STROKEWIDTH, 1);
            }
