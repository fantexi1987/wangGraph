...
export class wangFastOrganicLayout extends wangGraphLayout {
  useInputOrigin = true;
  resetEdges = true;
  ...
  execute(parent) {
    ...
    let initialBounds = this.useInputOrigin ? this.graph.getBoundingBoxFromGeometry(this.vertexArray) : null;
    ...
    for (let i = 0; i < this.vertexArray.length; i++) {
      ...
      this.cellLocation[i] = [];
      let id = wangObjectIdentity.get(vertex);
      this.indices[id] = i;
    ...
    try {
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < cells.length; j++) {
          ...
          let id = wangObjectIdentity.get(cells[j]);
