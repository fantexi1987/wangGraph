...
export class wangClient {
  static VERSION = '@wangGraph-VERSION@';
  ...
  static loadResources(fn, lan) {
    let pending = wangClient.defaultBundles.length;
    ...
    for (let i = 0; i < wangClient.defaultBundles.length; i++) {
      wangResources.add(wangClient.defaultBundles[i], lan, callback);
    }
  }
