...
export class wangGeometry extends wangRectangle {
  ...
  swap() {
    if (this.alternateBounds != null) {
      let old = new wangRectangle(this.x, this.y, this.width, this.height);
  ...
  rotate(angle, cx) {
    ...
    if (!this.relative) {
      let ct = new wangPoint(this.getCenterX(), this.getCenterY());
    ...
    if (this.sourcePoint != null) {
      let pt = wangUtils.getRotatedPoint(this.sourcePoint, cos, sin, cx);
      this.sourcePoint.x = Math.round(pt.x);
      this.sourcePoint.y = Math.round(pt.y);
    }

    if (this.targetPoint != null) {
      let pt = wangUtils.getRotatedPoint(this.targetPoint, cos, sin, cx);
    ...
    if (this.points != null) {
      for (let i = 0; i < this.points.length; i++) {
        if (this.points[i] != null) {
          let pt = wangUtils.getRotatedPoint(this.points[i], cos, sin, cx);
  ...
  equals(obj) {
    return (
      ...
      ((this.points == null && obj.points == null) ||
        (this.points != null && wangUtils.equalPoints(this.points, obj.points))) &&
